if (Meteor.isClient){
	Template.ProfileActive.helpers({
		...
		meetup: function () {
			...
			return Listing.find({ $or:[ { creator_id: Meteor.userId(), status: "Accepted" }, { offerAccepted: Offer.find({_id: this.offerAccepted})  } ] });
		},
		...
		defaultCheck: function () {
			...
			var id = Meteor.userId();
			var results = Listing.find({ $or:[ { creator_id: Meteor.userId(), status: "Accepted" }, { offerAccepted: Offer.find({_id: this.offerAccepted})  } ] });
