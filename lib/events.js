...
Template.ChatInput.events({
  'click .chatSend' : function () {
    ...
    // if ( !message ) {
    //   console.log( "Has not been filled out properly" );
    // }
...
// Manager Tab Highlighting
Template.Manager.events({
  'click .navTab': function (event) {
    $( ".navTab" ).removeClass( "navTabSelected" );
    $( event.currentTarget ).addClass( "navTabSelected" );
  }
});
...
Template.homeheaderpost.events({
 'click .navGlobal' : function (options) {
  ...
  Template.meetuprequest.events({
    'click .add': function (options) {
      var options = {
        listing_title: $( ".itemtitle" ).text(),
        img: $( ".mrImg img" ).attr( "src" ),
      ...
      if ( OfferValidate() ) {
        ...
        $( ".mrdiv" ).hide();
        $( ".sucess" ).append( "<h1 style='text-align: center;'> Offer Sent</h1>" );
        $( ".sucess" ).append( "<a href='/' class='errorm ph-button ph-btn-red'><i class='fa fa-home'></i></a>" );
        sAlert.success( "Offer Sucessfully Created" );
      };
    }}); 
