if (Meteor.isClient){
	...
	Template.UserProfileSettings.helpers({
		about: function () {
			return Meteor.users.find( { _id: id } ).fetch()[0].profile.about;
		},
		email: function () {
			return Meteor.users.find( { _id: id } ).fetch()[0].profile.email;
		},
		location: function () {
			return Meteor.users.find( { _id: id } ).fetch()[0].profile.location;
		},
	});
	...
	Template.chatLeft.helpers({
		yourMessages: function () {
			return Message.find({ receiver: Meteor.userId()
			}, {
				sort: {
					timestamp: -1
				},
		...
		image: function () {
			return Meteor.users.find( { _id: id } ).fetch()[0].picturesm;
		},

		conversations: function () {
			var conversationUnique = _.uniq(Message.find({}, {
				sort: {timestamp: 1}, fields: {username: true}
 ...
 //  		}
    ...
    Template.UserProfile.helpers({
    	...
    	upvotes: function () {
    		return Meteor.users.find( { _id: id } ).fetch()[0].profile.upvotes;
    	},
    	neutralvotes: function () {
    		return Meteor.users.find( { _id: id } ).fetch()[0].profile.neutralvotes;
    	},
    	downvotes: function () {
    		return Meteor.users.find( { _id: id } ).fetch()[0].profile.downvotes;
    	},
    	listing: function () {
